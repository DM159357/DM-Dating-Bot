    ...
    if args:
        ...
        if data:
            await update.message.reply_text(
                ...
                f"Gender: {data[2]}\n"
                f"Country: {data[3]}\n"
                f"🔗 Telegram: https://t.me/{BOT_USERNAME}"
            )
        else:
...
async def link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"🔗 Profile Link:\n{link}\n\n"
        "👉 Isse open karke koi bhi tumhara profile dekh sakta hai"
    )
...
async def relay(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        # 🔥 COPY MESSAGE (TEXT + MEDIA + EVERYTHING)
...
async def handle_profile(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if step == "name":
        ...
        await update.message.reply_text("✅ Registered! /chat")
...
if __name__ == "__main__":
    ...
    if is_registered(user):
        await update.message.reply_text("👫 Partner Matched 👫\n"
                "Welcome To DM Dating Bot ❤️\n"
                "/chat - Find partner\n"
                "/link - Share Profile Link\n"
                "/stop - End chat\n"
